...
const char *ValueToString(const Value v)
{
  ...
  {
  case TYPE_INT:
    return Str(v.value.i);
  case TYPE_FLOAT:
    return StrF(v.value.f);
  case TYPE_STRING:
    return v.value.s;
  case TYPE_LIST:
    return _ListToString(v.value.l);
  case TYPE_DICT:
    return _DictToString(v.value.h);
  default:
    return lstr_get("");
  }
}
...
const char *Trim(const char *str)
{
  const size_t len = strlen(str);
  size_t offset = 0;
  while (offset < len && isspace(str[offset]))
    ++offset;
  size_t count = len - offset - 1;
  while (count > 0 && isspace(str[offset + count]))
    --count;
