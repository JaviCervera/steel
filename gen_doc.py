...
def main():
    doc = "# STEEL Game Engine documentation\n\n"
  ...
  <head>
      ...
      <title>STEEL Game Engine documentation</title>
  </head>
  <body>
  ...
  """
    with open("_build/help/index.html", mode="w") as f:
        f.write(html)
    ...
    defs = sorted(
        [
            ...
            for d in root.findall(
                './compounddef/sectiondef/memberdef[@kind="define"]/name'
            )
        ]
    )
    funcs = sorted(
...
def parse_function(func):
    return Function(
        description=func.find("./detaileddescription/para").text.strip()
        if func.find("./detaileddescription/para") is not None
        else "",
        type=parse_type(
            (
                func.find("./type/ref")
                if func.find("./type/ref") is not None
                else func.find("./type")
            ).text
        ),
        ...
        return_desc=func.find(
            './detaileddescription/para/simplesect[@kind="return"]/para'
        ).text.strip()
        if func.find('./detaileddescription/para/simplesect[@kind="return"]/para')
        is not None
        else "",
    )
...
def parse_param(param, parameteritems):
    ...
    return Param(
        type=parse_type(
            (
                param.find("./type/ref")
                if param.find("./type/ref") is not None
                else param.find("./type")
            ).text
        ),
        name=name,
