...
class HaxeWrapperBuilder {
	public static function main():Void {
		final root = Xml.parse(File.getContent('_build/haxe/steel.xml')).firstElement();
		File.saveContent('_build/haxe/Steel.hx', getHaxeContent(root));
	}
	...
	private static function getHaxeContent(root:Xml):String {
		...
		return getHaxeClasses(classes) + '\n' + getHaxeSteelClass(constants, functions);
	}
	...
	private static function getHaxeExtends(className:String):String {
		return switch (className) {
			case 'AnimModel' | 'Camera' | 'Model' | 'Light' | 'Sprite':
				' extends Entity';
			default:
				'';
		};
	...
	private static function getHaxeSteelClass(constants:Iterable<Constant>, functions:Iterable<Function>):String {
		return '@:native("_G") extern class Steel {\n${getHaxeConstants(constants)}\n${getHaxeMethods(functions)}}\n';
	}
	...
	private static function getHaxeMethod(func:Function):String {
		final funcStr = Std.string(func);
		final funcSplit = funcStr.split('(');
		final funcName = funcSplit.shift();
		final funcRest = (funcSplit.length > 0) ? '(' + funcSplit.join('(') : '';
		switch (funcName) {
			case 'ASin' | 'ACos' | 'ATan' | 'ATan2' | 'RGB' | 'RGBA':
				return funcName.toLowerCase() + funcRest;
			default:
				return funcStr.substr(0, 1).toLowerCase() + funcStr.substr(1);
		}
	}

	private static function getClasses(functions:Iterable<Function>):Iterable<String> {
		final primitives = ['Void', 'Bool', 'Int', 'Float', 'String'];
